...
for hh = 1:length(sets)
    ...
    for qq = 1:5
        ...
        design1 = newall;
        design1(c:c+size-1,:) = [];
        target1 = newtargets(:,1);
        target1(c:c+size-1,:) = [];
        target2 = newtargets(:,2);
        target2(c:c+size-1,:) = [];
        target3 = newtargets(:,3);
        target3(c:c+size-1,:) = [];
        target4 = newtargets(:,4);
        target4(c:c+size-1,:) = [];
        target5 = newtargets(:,5);
        target5(c:c+size-1,:) = [];
        
        disp(['removing ',num2str(c),' to ',num2str(c+size-1),' from starting dataset'])
        data1 = newall(c:c+size-1,:);
        val1 = newtargets(c:c+size-1,1);
        val2 = newtargets(c:c+size-1,2);
        val3 = newtargets(c:c+size-1,3);
        val4 = newtargets(c:c+size-1,4);
        val5 = newtargets(c:c+size-1,5);
