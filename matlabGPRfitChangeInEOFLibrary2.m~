...
load('profileTrainingLibrary2Updated2.mat')

designParams = predictors(casenumbers,:);

load('profileBreaks.mat')

keepers = [1:44,46:63,65:121,123:161,163:209,211:250];

midBreakDist = middleBreakDist(keepers);
...
final(1:25,1) = ypred;% + predictors(validation,1);
final(1:25,2) = ypred2;% + predictors(validation,2);
final(1:25,3) = ypred3;% + predictors(validation,3);
final(1:25,4) = ypred4;% + predictors(validation,4);
final(1:25,5) = ypred5;% + predictors(validation,5);
final(1:25,6) = ypred6;% + predictors(validation,6);
final(1:25,7) = ypred7;% + predictors(validation,7);
final(1:25,8) = ypredd;
final(1:25,9) = ypreddist;
final(1:25,10) = ypredheight;

save('predictions.mat','ypred','ypred2','ypred3','ypred4','ypred5','ypred6','ypred7','ypredd','final','ypreddist','ypredheight')
figure
...
designParams = designParams(1:245,:);
midBreakDist = midBreakDist(1:245);
midBreakHeight = midBreakHeight(1:245);
sizedata = length(designParams);
[reorder] = randperm(sizedata);

sets = [49]; %,24,28,32,36,40,44,48,52,56,60,64,68,72,76,80,84,88,92,96];
totals = [245]; %,120,140,160,180,200,220,240,260,280,300,320,340,360,380,400,420,440,460,480];
...
%reorder = [1:95];
sets = [10, 15, 20, 25, 30, 35, 40, 45, 49];
totals = [50, 75, 100, 125, 150, 175, 200, 225, 245];
...
for hh = 1:length(sets)
    ...
    total = totals(hh);
    reorder = 1:total;%randperm(total);
...
%         xlabel('Xbeach PCs (m)')
%         ylabel('GPR PCs (m)')   
        clear gprMdl1 gprMdl2 gprMdl3 gprMdl4 gprMdl5 gprMdl6 gprMdl7 gprMdld
        c = c +size;
